...
static NSString * const STRequestManagerCacheDirectory = @"requestCacheDirectory";
static NSString * const STFileProcessingQueue = @"STFileProcessingQueue";

NSInteger const STResponseCancelError = -1;

@implementation STResponse @end




#pragma mark - STResponseCache

@interface STResponseCache : NSObject
...
@implementation STResponseCache {
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _queue = dispatch_queue_create([STFileProcessingQueue UTF8String], DISPATCH_QUEUE_CONCURRENT);
...
- (void)createCachesDirectory
{
    ...
    _cachePath = [cachePath stringByAppendingPathComponent:STRequestManagerCacheDirectory];
    BOOL isDirectory;
    if (![_fileManager fileExistsAtPath:_cachePath isDirectory:&isDirectory]) {
        ...
        if (!created) {
            NSLog(@"<STRequestManager> - create cache directory failed with error:%@", error);
        }
    }
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key
{
    ...
    dispatch_async(_queue, ^{
        ...
        if (!written) {
            NSLog(@"<STRequestManager> - set object to file failed");
        }
    });
}
...
- (void)removeAllObjects
{
    ...
    if (!removed) {
        NSLog(@"<STRequestManager> - remove cache directory failed with error:%@", error);
    }
}
    ...
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtURL:[NSURL URLWithString:_cachePath]
                                                   includingPropertiesForKeys:@[NSURLContentModificationDateKey]
                                                                      options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                        error:&error];
    if (error) {
        NSLog(@"<STRequestManager> - get files error:%@", error);
    }
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    if ([_fileManager fileExistsAtPath:filePath]) {
        ...
        if (!removed) {
            NSLog(@"<STRequestManager> - remove item failed with error:%@", error);
        }
    }
}
...
#pragma mark - STRequestManagerConfiguration

@implementation STRequestManagerConfiguration
...
@property (nonatomic) STResponseCache *cache;
...
{
    static STRequestManager *instance;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.cache = [[STResponseCache alloc] init];
...
- (AFHTTPRequestOperation *)GET:(NSString *)URLString
           ...
           configurationHandler:(STRequestManagerConfigurationHandler)configurationHandler
              completionHandler:(STRequestManagerCompletionHandler)completionHandler
{
                                         ...
                                         completionHandler:(STRequestManagerCompletionHandler)completionHandler
{
    ...
    // 对 request 做一层处理
    BOOL (^shouldStopProcessingRequest)(AFHTTPRequestOperation *, id userInfo, STRequestManagerConfiguration *) =  ^BOOL (AFHTTPRequestOperation *operation, id userInfo, STRequestManagerConfiguration *configuration) {
    ...
    // 对 response 做一层处理
    STResponse *(^handleResponse)(AFHTTPRequestOperation *, id,BOOL) = ^ STResponse *(AFHTTPRequestOperation *theOperation, id responseObject,BOOL isFromCache) {
    ...
    if (configuration.resultCacheDuration > 0 && [method isEqualToString:@"GET"]) {
        ...
        if (result) {
            ...
            if (!shouldStopProcessingRequest(operation, configuration.userInfo, configuration)) {
                ...
                NSError *error = [NSError errorWithDomain:@"取消请求" code:STResponseCancelError userInfo:nil];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *theOperation, id responseObject){
        STResponse *response = handleResponse(theOperation, responseObject,NO);
                 ...
                 completionBlock:(void (^)())completionBlock
{
    ...
    [operations enumerateObjectsUsingBlock:^(AFHTTPRequestOperation *operation, NSUInteger idx, BOOL *stop) {
        ...
        if (operationMethodParameters) {
            ...
            STRequestManagerCompletionHandler originCompletionHandler = [(STRequestManagerCompletionHandler) operationMethodParameters[@"completionHandler"] copy];
            
            STRequestManagerCompletionHandler newCompletionHandler = ^(NSError *error, id result, BOOL isFromCache, AFHTTPRequestOperation *theOperation) {
                if (!isFromCache) {
                    if (progressBlock) {
...
- (STRequestManagerConfiguration *)configuration
{
    if (!_configuration) {
        _configuration = [[STRequestManagerConfiguration alloc] init];
    }
...
- (AFHTTPRequestOperation *)createOperationWithConfiguration:(STRequestManagerConfiguration *)configuration request:(NSURLRequest *)request
{
